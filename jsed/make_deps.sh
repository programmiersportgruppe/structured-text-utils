...
install_autoconf_213() (
    if which -s autoconf-2.13
    then
        autoconf_213=autoconf-2.13
    elif which -s autoconf && autoconf --version | grep --quiet 'version 2.13'
    then
        autoconf_213=autoconf
    else
        if ! [ -x "deps/autoconf-2.13/autoconf" ]
        then
        ...
        autoconf_213="../../../autoconf-2.13/autoconf --macrodir=../../../deps/autoconf-2.13"
    fi
    echo "Using $autoconf_213 (relative to deps/js-1.8.5/js/src)"
)

lib_file=deps/js-1.8.5/js/src/dist/lib/libjs_static.a
install_libjs() (
    if [ -f "$lib_file" ]; then
        echo "libjs available at $lib_file"
        return
    fi
